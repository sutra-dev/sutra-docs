...
if uploaded_file is not None:
    try:
        ...
        embeddings = OpenAIEmbeddings(api_key=os.getenv("SUTRA_API_KEY"), base_url="https://api.numeric.tech/v2")
...
- **API Key**: The app requires a valid Sutra API key. Obtain one from [Numeric](https://www.numeric.tech) and set it in the `.env` file.
