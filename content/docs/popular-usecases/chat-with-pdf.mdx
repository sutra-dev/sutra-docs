...
- A valid **Sutra API key** (obtain from [Numeric's SUTRA API page](http://platform.two.ai))
...
- **API Keys**: Requires both a SUTRA API key (for the LLM) and an OpenAI API key (for embeddings). Obtain from [Numeric](http://platform.two.ai) and [OpenAI](https://platform.openai.com/account/api-keys), respectively, and store securely in Colab Secrets or a `.env` file.
...
- [Sutra API Documentation](http://platform.two.ai)
