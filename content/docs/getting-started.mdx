...
SUTRA is a family of large multi-lingual language models developed by Numeric, offering powerful AI capabilities through a simple, OpenAI-compatible API. This guide helps developers quickly integrate SUTRA-V2 for conversational AI, multilingual tasks, and more, from prototyping to production.
...
Contact [Numeric](https://www.numeric.tech) for enterprise API access. All API requests require a SUTRA API key. Store your key securely as an environment variable:
...
```bash
curl -X POST "https://api.numeric.tech/v2/chat/completions" \
...
- Invalid API Key: Verify at [Numeric](https://www.numeric.tech).
...
- **Email**: [Numeric](https://www.numeric.tech)
- **Documentation**: [SUTRA Docs](/docs)
