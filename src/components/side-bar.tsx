...
// Bypass strict TypeScript type checking for components with JSX compatibility issues
const createComponent = (Component: any, props: any, ...children: any) => {
...
export default function ArticleLayout() {
  ...
  const cts = group === "docs" ? contents : cookbooks;
  ...
  return (
    <div className={cn("fixed top-0")}>
      <aside
        className={cn(
          "md:transition-all",
          "border-r border-lines top-[55px] md:flex hidden md:w-[268px] lg:w-[286px] overflow-y-auto absolute h-[calc(92dvh-7px)] flex-col justify-between w-[var(--fd-sidebar-width)]"
        )}
      >
        <div>
          <button
          ...
          >
            {createComponent(Search, { className: "size-4 mx-0.5" })}
              ...
              {cts.map((item, index) =>
                item?.isPage ? (
                  ...
                  <AsideLink
                    ...
                    startWith="/docs"
                    title={item.title}
                    className="break-words text-nowrap w-[--fd-sidebar-width] [&>div>div]:hover:!bg-fd-muted text-sm px-5 py-2.5"
                    activeClassName="[&>div>div]:!bg-fd-muted"
                  >
                    <div className="min-w-4">
                      {createComponent(item.Icon, {
                  ...
                  <div key={item.title}>
                    ...
                    >
                      {createComponent(item.Icon, { className: "size-5" })}
                      <span className="grow">{item.title}</span>
                      {item.isNew && <NewBadge />}
                      <motion.div
                        animate={{ rotate: currentOpen === index ? 180 : 0 }}
                      >
                        {createComponent(ChevronDownIcon, {
                          className: cn(
...
const tabs = [
  ...
  },
  {
    ...
    title: "Cookbooks",
    description: "cookbooks and guides",
  },
];
...
function SidebarTab({
  ...
  return createComponent(
    ...
    createComponent(
      SelectTrigger,
      { className: "h-16 border border-b border-none rounded-none px-5" },
      selected ? (
